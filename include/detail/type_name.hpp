...
#define CPPALLOC_FUNC_START                                                    \
	sizeof("detail::const_string_typeh cppalloc::detail::type_name() [T = ") - 1
...
template <typename T> constexpr detail::const_string_typeh type_name() {
	return detail::const_string_typeh{CPPALLOC_FUNC_NAME}.substring(
	    CPPALLOC_FUNC_START, CPPALLOC_FUNC_END);
}

template <typename T> constexpr std::uint32_t type_hash() {
	return detail::const_string_typeh{CPPALLOC_FUNC_NAME}
	    .substring(CPPALLOC_FUNC_START, CPPALLOC_FUNC_END)
	    .hash();
}
} // namespace detail
