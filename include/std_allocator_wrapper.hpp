...
class std_allocator_wrapper<T, underlying_allocator, true>
    : public std::allocator<T>, public underlying_allocator {
public:
	//    typedefs
	using pointer = typename std::allocator_traits<std::allocator<T>>::pointer;
	using const_pointer =
...
public:
	...
	inline pointer allocate(size_type cnt, const_pointer = 0) const {
		pointer ret = reinterpret_cast<pointer>(underlying_allocator::allocate(
		    static_cast<underlying_size_t>(sizeof(T) * cnt)));
		return ret;
	}

	inline void deallocate(pointer p, size_type cnt) const {
		underlying_allocator::deallocate(
		    p, static_cast<underlying_size_t>(sizeof(T) * cnt));
	}
	//    construction/destruction
	...
	inline void destroy(pointer p) const { reinterpret_cast<T*>(p)->~T(); }
	inline bool operator==(const std_allocator_wrapper& x) { return true; }
	inline bool operator!=(const std_allocator_wrapper& x) { return false; }
};
